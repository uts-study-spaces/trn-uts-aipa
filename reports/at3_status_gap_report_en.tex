...
\begin{enumerate}
  ...
  \item \textbf{Prepare the demo:} train the models locally, launch Streamlit, test the Security Incident example, and keep screenshots as backup.
  ...
  \item \textbf{Avoid overclaiming:} describe LLMs and GenAI as future work. A small transformer-embedding benchmark now exists, but it is optional, sampled, and not part of the Streamlit app.
...
\addlinespace
AI workflow and implementation & \done & Report + presentation & \textbf{Frame as evidence.} The repo includes notebooks 01--05, reusable modules in \texttt{src/customer\_support\_ai/}, model training, evaluation, report assets, and Streamlit. Use the workflow diagram in slides and describe the design decisions versus implementation details in the report. \\
\addlinespace
...
Add backup screenshots & 15 minutes & Reduces risk if Streamlit fails during the presentation. \\
...
Mention transformer embeddings in the presentation & 10 minutes & Strengthens the SLO4 discussion. Recommended wording: ``We added an optional sampled transformer-embedding benchmark for the report, but the demo keeps Linear SVM because it is stronger and more explainable in our current results.'' \\
Refine transformer benchmark if time allows & 2--4 hours & Optional for the final report. Try larger samples, class balancing, or Linear SVM on embeddings. Keep it out of the live app unless it clearly outperforms the current pipeline. \\
Update README with exact demo command & 10 minutes & Helps every group member run the app reliably. \\
...
\textbf{Presentation action:} mention it briefly as report evidence or future refinement, not as the main system.
